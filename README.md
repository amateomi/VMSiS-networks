...
- Тетрады вида `:0000:` можно заменить на `:0:`
- последовательности из больше чем одного нуля можно не писать, оставив `::` на их месте (можно применить тольк один раз, ибо иначе невозможно было бы определить, сколько нулей пропущено в этих последовательностях)
...
Вместо маски IPv6 имеет префикс подсети, по смыслу и внешне похож на IPv4
маску в CIDR нотации. Стандартный префикс подсети равен 64.
...
середину байтов `FF FE`. Если у сетевого интерфейса нет MAC-адреса (например в
топологиях Point-to-Point), то можно использовать какой-нибудь другой
