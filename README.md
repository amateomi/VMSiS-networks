...
2. bootstrap ищет образ IOS по специальной строке в конфигурации либо по значению переменной BOOT. Если не нашел, то поиск продолжается в подсистеме Flash памяти, выбирая первый попавшийся образ. Если образов не найдено, то запускается интерпретатор командной строки ROMMON, позволяющий скопировать образ в Flash.
...
Адрес подсети всегда самый нижний адрес из диапазона адресов подсети.
...
### Консольный (cmd)
Для просмотра IP-адреса можно использовать утилиту ipconfig, а для настройки -
netsh.
...
6. При одинаковой метрике в разных реализациях могут применяться различные алгоритмы балансировки нагрузки, так или иначе будет выдано предпочтение одному из маршрутов (в некоторых реализациях совпадение метрик недопустимо)
...
Главной задачей динамической маршрутизации является передача маршрутов к
подсетям между узлами с целью нахождения абонентов и оптимизации трафика.
...
- По вектору расстояний (DVA) (опираются на таблицы, поддерживаемые всеми маршрутизаторами и содержащие сведения о кратчайших известных путях к каждому из возможных адресатов и о том, какое соединение следует при этом использовать. Для обновления данных в этих таблицах производиться обмен информацией с соседними маршрутизаторами. Базовым является алгоритм Беллмана-Форда)
...
1. IP-пакет на исходной станции передается на канальный уровень (инкапсулируясь, например, в кадр Ethernet)
...
5. Если пакет предназначен станции в другой подсети, то он будет передан согласно таблице маршрутизации (возможно с повторением пункта `2` для адреса шлюза)
...
(например, HDD), а с накопителя на некоторой удаленной станции.

Структурно в удаленной загрузке можно выделить 2 элемента:
...
- IPv4: протоколы BOOTstrap Protocol (BOOTP) -> Dynamic Host Configuration Protocol (DHCP) -> Preboot eXecution Environment (PXE) с соответствующим ПО
...
Для поддержки удаленной загрузки в качестве загружаемой станции необходимо
иметь DHCP и TFTP клиенты (если речь идет о PXE), а загружающей станции - DHCP
и TFTP серверы.
...
Link-local Unicast имеет вид `FE80::/10` т.е. первые 10 бит это 1111_1110_10,
...
Если у сетевого интерфейса нет MAC-адреса (например в топологиях Point-to-Point),
то можно использовать какой-нибудь другой уникальный идентификатор, например,
чужой MAC-адрес или UUID роутера.

Пакеты с адресом Link-local Unicast не могут покинуть свой линк.

Для организации внутренней сетевой иерархии на каком-нибудь предприятии можно
...
- Stateful (централизованное управление, адреса и другие параметры передаются по специальному протоколу, базируется на DHCPv6)
...
Значения для этих таймеров могут быть изменены на соответствующие значения
полей RA пакетов.

## 62. Взаимодействие по протоколу Netboot6

Netboot6 - протокол удаленной загрузки для IPv6, аналог PXE в IPv4. Также
как и PXE использует TFTP и DHCP, но уже не DHCPv4, а DHCPv6.
...
DHCPv6-сервер способен выдавать как постоянные так и врЕменные адреса. По
аналогии с SLAAC постоянные адреса имеют Valid Lifetime и Preferred Lifetime.
...
Проверка адресов происходит в порядке указаных правил. Если победитель не выявлен,
то дальнейший выбор зависит от реализации.
...
В IPv6 появилась оптимизация вышеописанного подхода, позволяющая
передавать пакеты между мобильным хостом и каким-то сервером в
...
- Unspecified `::/128` (адрес всех глобальных сетей)
- Loopback `::1/128` (адрес сетевого интерфейса-заглушки)

Изменения в маршрутизации. Специальные соглашения в IPv6-маршрутизации:
- Маршрут по умолчанию `::/0`
- Маршрут к большей чем линк подсети `X:...:X/N`, N < 64
- Маршрут к подсети размером с линк `X:X:X:X/64`
- Маршрут к одному сетевому интерфейсу `X:X:X:X:X:X:X:X/128`
...
- Intrusion Detection Systems (проверяют трафик по сигнатурам на вредоносность с последующим оповещением при обнаружении оного. Обычно трафик не пропускается через них на прямую, а дублируется)
...
Реализацию прокси можно поделить на 3 направления:
- Host-based (программные решения на базе универсальных ОС (Windows, дистрибутивы Linux, Android) работающие на обычном аппаратном обеспечении. Могут реализовываться на уровне ядра ОС и на прикладном уровне)
- Integrated (программно-аппаратные решения на базе специализированных сетевых ОС (Cisco IOS), работающих на сетевом оборудовании (маршрутизаторы и т.п.). Имеется возможность расширения за счет аппаратных модулей)
- Appliance-based (полностью специализированные аппаратные решения (Cisco ASA). Они сосредоточены на обеспечении безопасности и из называют аппаратными сетевыми экранами)

Решение различных задач прокси более уместно на определенных уровня модели
OSI, а также в различных категориях ПО:
- Ядра ОС
- Системное ПО
- Прикладное ПО
...
Начиная с Windows Server 2003 в ядро интегрирован Windows Firewall.
Позиционируется как базовый персональный сетевой экран.

В современных редакциях Windows (Windows 10, Server 2016, 2019, ...)
Windows Firewall + Windows Defender + Windows Action Center переносят
в отдельное системное приложение Windows Security.

В настольных редакциях Windows есть две упрощенные версии NAT: Network
Bridge и Internet Connection Sharing (вытесняют сервис Routing and Remote
Access).

Network Bridge позволяет объединить два возможно разнородных сегмента с
целью эмуляции одного сегмента.

Internet Connection Sharing позволяет нескольким пользователям из одной
подсети совместно использовать (в режиме разделения времени) один сетевой
интерфейс из другой подсети, обычно с целью доступа в Internet.

Полноценная поддержка NAT с графическим интерфейсом доступна только в серверных
редакциях Windows в составе Routing and Remote Access.

Кроме того, очень широко применяют пакеты сторонних производителей.

## 85. Пакет IP Tables

IP Tables - стандартная пакет в Linux, позволяющий осуществлять настройку
сетевого экрана (firewall) и NAT.

Для нормальной работы IP Tables должны быть включены некоторые опции ядра.

Фильтрация стоится на основе правил (rules) - условий, определяющих попадает
ли пакет под правило, и действие, которое необходимо осуществить в случае
выполнения условия.

Правила могут объединяться в цепочки (chains) и образовывать сложную иерархию.

Управление осуществляется через утилиту iptables. Общий формат
использования утилиты: `iptables [-t table] command [match] [target/jump]`

Примеры таблиц:
- `filter`
- `mangle` (нужна для внесения изменений в заголовки пакетов)
- `nat`
...
Параметры действий (targets):
- ACCEPT (пакет прекращает движение по цепочке и считается пропущенным)
- DROP (пакет отбрасывается)
- REJECT (DROP с посылкой ICMP-сообщения о недостижимости)
- LOG (протоколирование пакета)
...
Переходы (jump) позволяют передавать пакет другим цепочкам.

## 86? Полнофункциональные прокси на базе Windows и Linux

Среди полнофункциональных прокси-серверов для Windows можно выделить ПО от
сторонних производителей: Qbik WinGate и KerioControl.

Де-факто стандартным прокси-сервером в ОС семейства UNIX, в том числе Linux,
является пакет Squid со стандартным конфигурационным файлом
`/etc/squid/squid.conf`.

## 87. Поддержка NAT в IOS

Cisco IOS поддерживает все теоретические варианты NAT. Статические и
динамические преобразования совместимы.

Обязательно нужно привязать сетевые интерфейсы к внутренней сети
(inside) и сети публичного доступа (outside) командами `ip nat inside` и
`ip nat outside` соответственно.

Важно правильно понимать Cisco-терминологию, связанную с адресацией
при NAT-преобразованиях:
- Inside local (адрес расположенной в сети inside станции как он виден в сети inside)
- Inside global (адрес расположенной в inside-сети станции как он виден в сети outside)
- Outside local (адрес расположенной в сети outside станции как он виден в сети inside)
- Outside global (адрес расположенной в сети outside станции как он виден в сети outside)

Вся конфигурация NAT в IOS осуществляется командой `ip nat` с множеством
различных аргументов.

По умолчанию строка NAT-таблицы считается валидной 24 часа для TCP, 5
минут для UDP и 1 минуту для ICMP.

Для просмотра NAT-таблицы используют команду `show ip nat translations`.
